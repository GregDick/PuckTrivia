...
- [ ] In landscape, when **no Next button is shown** (i.e., before the player has answered), the question text is **vertically centered** in the left column.
- [ ] In landscape, when an answer has been selected and the **Next** button is shown, the Next button sits at the **bottom of the left column** (the question column, not the answer column), and the question text **shifts up / top-aligns** in that column to make room for it. The Next button is fully visible and tappable and is not clipped off the bottom of the screen.
...
- Left column holds the question text **and the Next button**. When the Next button is absent (unanswered), vertically center the question text within the column for balance. When the Next button is present (answered), the question moves to the top of the column and the Next button is anchored at the bottom of the same column — so the question reads as upper-aligned with the Next button beneath it, not overlapping. Keep `headlineSmall` styling on the question and the existing `OutlinedButton` "Next" styling (`fillMaxWidth`, `bodyLarge` label).
...
- **Next button lives in the left (question) column, not the answer column.** This is the fix for the bug where the Next button clipped off the bottom of the screen in landscape (it previously sat below the scrollable answers in the right column). Lay the left column out so the question and the Next button cannot overlap: e.g., a left `Column` with the question text and an answered-only Next button anchored at the bottom (a `Spacer(weight(1f))` or `Arrangement.SpaceBetween` between question and button), or a `Box` whose question alignment switches from `Center` (unanswered) to `TopCenter` (answered) with the Next button aligned `BottomCenter`. Keep the question's `headlineSmall` and the Next `OutlinedButton`/`bodyLarge` styling unchanged.
- Vertical alignment is conditional on `answered`: question vertically centered when the Next button is hidden; question top-aligned with Next pinned to the bottom when `answered` is true and the Next button shows. Reuse the existing `answered` flag — no new state.
...
### QA / Testing Notes
- Manual: rotate to landscape on a phone mid-question; verify left=question, right=answers, no overlap, all buttons tappable. Before answering, confirm the question is vertically centered in the left column. After answering, confirm the question shifts up and the Next button appears at the bottom of the left column (not in the answer column) and is fully visible.
...
- **Question alignment shift on answer:** when the player answers, the question moves from vertically centered to top-aligned and the Next button appears below it. Verify this transition is not jarring and the question never overlaps the Next button at any height.
- **Long question text with the Next button present:** the left column must keep the Next button reachable (scroll or sensible sizing) rather than letting a long question push it off-screen.
