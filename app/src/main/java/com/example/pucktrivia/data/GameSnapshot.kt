 ...
 * game survives OS-initiated process death. It is [Serializable] so the framework can serialise it
 * directly into the saved-state [android.os.Bundle] — no hand-rolled codec. (`@Parcelize` would be
 * the idiomatic choice, but the parcelize compiler plugin does not activate under AGP 9's built-in
 * Kotlin — see https://issuetracker.google.com/issues/389977429.)
 *
 ...
 * whenever the class shape changes, making deserialisation of a stale snapshot throw. The restore
 * path catches that and treats it as "no active game" — the same fail-safe schema-versioning
 * contract [HighScoreCodec] implements by hand.
 */
